...
| CALC-2 | 🟡 하 | 소수/음수/NaN 금액을 조용히 흡수(절삭·무시) — 검증을 호출부에 전가 | ✅ 해결(2026-06-04) | `app/lib/validation.ts`, `app/lib/actions.ts` |
...
- **상태**: ✅ **해결(2026-06-04, dev/M1-#3)**. `app/lib/validation.ts`에서 금액(0 이상 유한 정수)·라벨·연/월·enum·할부 회차를 서버측 검증, 모든 Server Action(`actions.ts`)이 저장 전 검증. 통합테스트로 음수/소수/비숫자/회차초과/잘못된 kind 거부 확인.
